...
import { 
  ...
  FaJava, FaChartLine, FaDatabase, FaGlobe, FaBars 
} from 'react-icons/fa';
import { 
    ...
    const observer = new IntersectionObserver((entries) => {
      if (entries[0].isIntersecting) {
        ...
        const animate = (currentTime) => {
          ...
          else { setExp(4.5); setProjectsCount(12); }
        };
  ...
  return (
    <div className="App">
        ...
        <div className="hero-content-wrapper">
          <div className="hero-text fade-in-up">
            ...
            <div className="social-icons">
              <a href="https://github.com/lakmewang" target="_blank" rel="noreferrer"><FaGithub className="social-icon" size={35} /></a>
              <a href="https://lk.linkedin.com/in/lakmewan-gayantha-413365250" target="_blank" rel="noreferrer"><FaLinkedin className="social-icon" size={35} /></a>
              <a href="https://www.facebook.com/people/Lakmewan-Gayantha/100011020021151" target="_blank" rel="noreferrer"><FaFacebook className="social-icon" size={35} /></a>
              <a href="https://blazevex.com/" target="_blank" rel="noreferrer">
                <FaGlobe className="social-icon blinking-globe" size={35} style={{ color: '#ff0073', filter: 'drop-shadow(0px 0px 8px rgba(255, 0, 0, 0.8))' }} />
              </a>
            </div>
          ...
          <div className="code-window fade-in-up hide-on-mobile">
      ...
      <div className="container">
        ...
        <section id="about" className="reveal">
          ...
          <div className="about-container">
            <div className="about-text">
               ...
               <h2 className="green-text section-title-mobile" style={{fontSize: '2.5rem', marginBottom: '25px'}}>Developing with <br/>Passion & Purpose</h2>
               ...
               <div style={{ marginTop: '30px', padding: '20px', background: 'rgba(255, 255, 255, 0.03)', borderRadius: '8px', borderLeft: '4px solid #4ade80' }}>
               ...
               <div className="stats-container" style={{display:'flex', gap:'40px', marginTop:'40px'}} ref={statsRef}>
                  <div><h4 className="pink-text" style={{fontSize: '1.8rem'}}>{exp}+</h4><p style={{fontSize: '0.8rem', color: '#666'}}>YEARS EXP.</p></div>
                  <div><h4 className="pink-text" style={{fontSize: '1.8rem'}}>{projectsCount}+</h4><p style={{fontSize: '0.8rem', color: '#666'}}>PROJECTS</p></div>
               </div>
            </div>
            <div className="about-image">
               <div className="about-img-box"><img src="images/profile.png" alt="Profile" /></div>
            </div>
          </div>
        </section>
        ...
        <section id="skills" className="reveal">
          ...
          <div className="skills-slider" style={{ marginBottom: '30px' }}>
            <div className="slide-track">
              {skillsRow1.map((skill, index) => (<div className="skill-card" key={`1a-${index}`}>{skill.icon}<span>{skill.name}</span></div>))}
              {skillsRow1.map((skill, index) => (<div className="skill-card" key={`1b-${index}`}>{skill.icon}<span>{skill.name}</span></div>))}
            </div>
          </div>
          <div className="skills-slider">
            <div className="slide-track slide-track-reverse">
              {skillsRow2.map((skill, index) => (<div className="skill-card" key={`2a-${index}`} style={{ width: '180px', minWidth: '180px', height: '80px', justifyContent: 'center' }}><span style={{ marginTop: '0', fontSize: '1.1rem', fontWeight: '500' }}>{skill.name}</span></div>))}
              {skillsRow2.map((skill, index) => (<div className="skill-card" key={`2b-${index}`} style={{ width: '180px', minWidth: '180px', height: '80px', justifyContent: 'center' }}><span style={{ marginTop: '0', fontSize: '1.1rem', fontWeight: '500' }}>{skill.name}</span></div>))}
            </div>
          </div>
        </section>
          ...
          <div className="projects-grid">
            {projects.map((project) => (
              <div className="project-card" key={project.id} onClick={() => setSelectedProject(project)}>
                <div className="project-img-wrapper"><img className="project-img" src={project.img} alt={project.title} /></div>
                <div className="project-info">
                  <h3>{project.title}</h3>
                  <p>{project.desc.substring(0, 80)}...</p>
                  <div className="tech-stack">
                    {project.stack.map((tech, i) => (<span className="tech-tag" key={i}>{tech}</span>))}
                  </div>
        ...
        <section id="contact" className="reveal">
          <div className="contact-wrapper">
            <div className="contact-form-box" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              <h3 style={{ fontSize: '2.5rem', marginBottom: '20px' }}>Interested in <span className="green-text" style={{color: 'var(--ocean-light)'}}>Collaboration?</span></h3>
              <p style={{ marginBottom: '35px', color: '#a0a0c0', fontSize: '1.1rem', lineHeight: '1.6', maxWidth: '90%' }}>
                I'm currently looking for new opportunities and my inbox is always open. 
                Whether you have a question or just want to say hi, I'll try my best to get back to you!
              </p>
              <div className="btn-group">
                <a href="mailto:lakmewang@gmail.com" className="btn btn-fill">SAY HELLO <FaEnvelope style={{ marginLeft: '10px' }}/></a>
                <a href="https://drive.google.com/file/d/180qyTdEfeCEgiRMidHo2y_RJhq5msPZJ/view?usp=sharing" className="btn btn-outline">VIEW RESUME <FaDownload style={{ marginLeft: '10px' }}/></a>
              </div>
            </div>
            <div className="contact-info-box">
               ...
               <div className="info-list" style={{ display: 'flex', flexDirection: 'column', gap: '30px', justifyContent: 'center', width: '100%' }}>
                 <div className="info-item" style={{ display: 'flex', alignItems: 'center', gap: '20px' }}><div className="s-icon"><FaEnvelope /></div><span style={{ fontWeight: '500', fontSize:'1.1rem' }}>lakmewang@gmail.com</span></div>
                 <div className="info-item" style={{ display: 'flex', alignItems: 'center', gap: '20px' }}><div className="s-icon"><FaPhone /></div><span style={{ fontWeight: '500', fontSize:'1.1rem' }}>+94 76 8411 372</span></div>
                 <div className="info-item" style={{ display: 'flex', alignItems: 'center', gap: '20px' }}><div className="s-icon"><FaMapMarkerAlt /></div><span style={{ fontWeight: '500', fontSize:'1.1rem' }}>Rajagiriya, Sri Lanka</span></div>
                 <div className="social-row" style={{ marginTop: '30px', display: 'flex', gap: '20px' }}>
                   <a href="https://github.com/lakmewang" target="_blank" rel="noreferrer" className="s-icon"><FaGithub style={{fontSize:'1.2rem'}}/></a>
                   <a href="https://lk.linkedin.com/in/lakmewan-gayantha-413365250" target="_blank" rel="noreferrer" className="s-icon"><FaLinkedin style={{fontSize:'1.2rem'}}/></a>
                   <a href="https://www.facebook.com/people/Lakmewan-Gayantha/100011020021151" target="_blank" rel="noreferrer" className="s-icon"><FaFacebook style={{fontSize:'1.2rem'}}/></a>
                 </div>
               </div>
            </div>
          </div>
        ...
        <footer style={{ borderTop: '1px solid #1f1f35', padding: '30px 0', marginTop: '50px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', color: '#666', fontSize: '0.9rem' }}>
          <span>© 2026 Lakmewan G</span>
        </footer>

        {/* --- SCROLL TO TOP BUTTON --- */}
        <button className={`scroll-to-top-btn ${showScrollBtn ? 'visible' : ''}`} onClick={scrollToTop}><FaArrowUp /></button>
      </div>
      ...
      {selectedProject && (
        <div className="modal-overlay" onClick={() => setSelectedProject(null)}>
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <button className="modal-close-btn" onClick={() => setSelectedProject(null)}><FaTimes /></button>
            <div className="modal-body">
              <div className="modal-img-container"><img src={selectedProject.img} alt={selectedProject.title} className="modal-img" /></div>
              <div className="modal-text">
                <h2 className="modal-title">{selectedProject.title}</h2>
                <h4 className="modal-subtitle">{selectedProject.subtitle}</h4>
                <p className="modal-desc">{selectedProject.desc}</p>
                <div className="modal-features">
                  {selectedProject.features.map((feature, index) => (<div className="modal-feature-item" key={index}><FaCheckCircle color="var(--ocean-main)" /> {feature}</div>))}
                </div>
                <div className="tech-stack" style={{marginBottom:'30px'}}>
                  {selectedProject.stack.map((tech, i) => (<span className="tech-tag" key={i}>{tech}</span>))}
                </div>
                <div className="modal-actions">
                  <a href={selectedProject.links.github} className="btn btn-outline" target="_blank" rel="noreferrer"><FaGithub style={{marginRight: '8px'}}/> GitHub</a>
                  <a href={selectedProject.links.live} className="btn btn-fill" target="_blank" rel="noreferrer"><FaExternalLinkAlt style={{marginRight: '8px'}}/> Live Demo</a>
                </div>
              </div>
            </div>
          </div>
